{% macro oracle__gen_joined_metrics_cte(leaf_set,expression_set,ordered_expression_set,grain,dimensions,calendar_dimensions,secondary_calculations,relevant_periods) %}
...
{% endif %}
    ...
    ,first_join_metrics as (
        ...
        {% for metric_name in leaf_set %}
			{% if loop.first %}
				{{metric_name}}__final.date_{{grain}}
			{% endif %}
    	{% endfor %}			
        ...
        from 
            ...
            {# can't do set inside loop with jinja so hack using list #} 
            {% set metric_list = [] %}
            {% for metric_name in leaf_set %}
                {% if loop.first %}
                    {{metric_name}}__final                    
                    {% set foo = metric_list.append(metric_name ~ '__final') %}                    
                {% else %}
                    left outer join {{metric_name}}__final 
                    on {{ metric_list[0] }}.date_{{grain}} = {{metric_name}}__final.date_{{grain}} 
                	
                	{% for calendar_dim in calendar_dimensions %}
                    	and {{ metric_list[0] }}.{{ calendar_dim }} = {{metric_name}}__final.{{ calendar_dim }} 
                	{%- endfor %}
                	
                	{% for dim in dimensions %}
                    	and {{ metric_list[0] }}.{{ dim }} = {{metric_name}}__final.{{ dim }}
                	{%- endfor %}                       
                {% endif %}
            {% endfor %} 
    )
