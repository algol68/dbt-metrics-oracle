{% macro oracle__gen_metric_cte(metric_name, grain, dimensions, secondary_calculations, start_date, end_date, relevant_periods, calendar_dimensions) %}
...
,{{metric_name}}__final as (
    ...
    left outer join {{metric_name}}__aggregate
        on {{metric_name}}__spine_time.date_{{grain}} = {{metric_name}}__aggregate.date_{{grain}} 
                {% for calendar_dim in calendar_dimensions %}
                    and {{metric_name}}__spine_time.{{ calendar_dim }} = {{metric_name}}__aggregate.{{ calendar_dim }} 
                {%- endfor %}
                {% for dim in dimensions %}
                    and {{metric_name}}__spine_time.{{ dim }} = {{metric_name}}__aggregate.{{ dim }}
                {%- endfor %}
